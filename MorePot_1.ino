...
void runAuto() {
  if (newHour == hour() && newMinute == minute() && newDay ==day() &&  timeflag == 0) {
    ...
    if (water_level < 0) {
      if (SwitchV5 == 1) {
        Blynk.notify("The water LEVEL is to LOW. Fill your Watertank and Restart the MorePot!");
...
void setup()
{
  ...
  if (!wifiManager.autoConnect("MorePot", "7754")) {
  ...
  Serial.println("Connected MorePot Watering System :)");   //if you get here you have connected to the WiFi
  ticker.detach();
  //turn LED off
